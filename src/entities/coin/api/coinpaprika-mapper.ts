...
function toFiniteNumber(value: number | null | undefined, fallbackValue = 0) {
  return typeof value === 'number' && Number.isFinite(value) ? value : fallbackValue;
}

function toOptionalFiniteNumber(value: number | null | undefined) {
...
function inferCategory(ticker: CoinPaprikaTicker): Coin['category'] {
  ...
  const marketCap = toFiniteNumber(ticker.quotes.USD.market_cap);
...
export function mapCoinPaprikaTickerToCoin(ticker: CoinPaprikaTicker): Coin {
  ...
  return {
    ...
    change24h: toFiniteNumber(quote.percent_change_24h),
    change7d: toOptionalFiniteNumber(quote.percent_change_7d),
    marketCap: toFiniteNumber(quote.market_cap),
    volume24h: toFiniteNumber(quote.volume_24h),
    circulatingSupply: toFiniteNumber(ticker.circulating_supply),
    maxSupply: toOptionalFiniteNumber(ticker.max_supply),
    category,
